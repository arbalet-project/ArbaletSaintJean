...
int save_x_back;
int save_y_back;
int taille_serpent;//penser à initialiser 
unsigned int compteur;

void nouveauFruit(){
    int alea = random(TAILLE - taille_serpent);
    int i = 0;
    int compteurNew = 0;
    boolean fini = false;
...
void decaler(int directionSnake){  
  ...
  color_that_case(save_x_front,save_y_front,100,50,0);
  
  switch(directionSnake){
        case 0://gauche
          save_y_front = (save_y_front+1)%(LARGEUR+1);
          break;
          
        case 1: //droite
          if(save_y_front-1 < 0)
          {
            save_y_front = LARGEUR;
          }
          else{
            save_y_front -= 1;
          }
          break;
        case 2://haut
          save_x_front = (save_x_front+1)%(HAUTEUR+1);
          break;
        case 3://bas
          if(save_x_front-1 < 0)
          {
            save_x_front = HAUTEUR;
          }
          else{
            save_x_front -= 1;
          }
    ...
    if(positionspossibles[calculer(save_x_front,save_y_front)] == 'f'){
      positionspossibles[calculer(save_x_front,save_y_front)] = 's';
      color_that_case(save_x_front,save_y_front,0,0,0);
      nouveauFruit();
      compteur= compteur -10;
    }
    else if(positionspossibles[calculer(save_x_front,save_y_front)] == 's'){
      gameRunning = false;
    }

    else{
      color_that_case(save_x_back,save_y_back,0,0,0);
      positionspossibles[calculer(save_x_back,save_y_back)] = 'r';
      positionspossibles[calculer(save_x_front,save_y_front)] = 's';
      }
      color_that_case(save_x_front,save_y_front,255,0,0);
...
{  
  ...
  positionspossibles[calculer(HAUTEUR/2-2,LARGEUR/2-1)] = 's';
  strip.setPixelColor(calculer(HAUTEUR/2-2,LARGEUR/2-1), 100, 50, 0);
  positionspossibles[calculer(HAUTEUR/2-1,LARGEUR/2-1)] = 's';
  strip.setPixelColor(calculer(HAUTEUR/2-1,LARGEUR/2-1), 100, 50, 0);
  positionspossibles[calculer(HAUTEUR/2,LARGEUR/2-1)] = 's';
  strip.setPixelColor(calculer(HAUTEUR/2,LARGEUR/2-1), 100, 50, 0);
